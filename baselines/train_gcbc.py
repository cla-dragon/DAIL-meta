...
from BabyAI.babyai.new_missions import *
...
from agents.agent_gcbc import BaseAgent
...
def get_config():
    ...
    parser.add_argument("--run_name", type=str, default="Synth", help="Run name description")
    ...
    parser.add_argument("--data_path", type=str, default="data/data_Synth_medium_Bot12500_IL25000_Random40000_clip.pkl", help="Path to data")
    ...
    parser.add_argument("--max_time_steps", type=int, default=80, help="")

    parser.add_argument("--use_film", type=bool, default=True, help="")
    parser.add_argument("--use_bptt", type=bool, default=False, help="")
    parser.add_argument("--use_fliter", type=bool, default=False, help="fliter the successful trajectories")
...
def evaluate(config, policy:BaseAgent, eval_runs=100, ood_test=True): 
    """
...
def train(local_rank, world_size, config, run_num, dataset, metric_queue=None):
    ...
    batches = 0
    fliter_postfix = 'fliter' if config.use_fliter else 'nofliter'
    medium_postfix = 'medium' if 'medium' in config.data_path else 'expert'
    if local_rank == 0:
        wandb.init(project="Babyai-Base1.1",name=f'run-{config.run_name}-{medium_postfix}-{fliter_postfix}-seed{config.seed}' , config=config)
        # writer = SummaryWriter()
        
    agent = BaseAgent(config, device)
    ...
    for i in range(1, config.episodes+1):
        ...
        for batch_idx, experience in enumerate(dataloader):
            ...
            # if (batches % config.eval_every == 5) and (local_rank == 0 or local_rank == 1):
            #     eval_metrics = {}
            #     agent.network.eval()
            ...
            #             metrics[key] = value
            #     print("Episode: {} | {}-Reward: {} | Win Rate: {} | Polciy Loss: {} | Batches: {}"
            #           .format(i, posfixs[local_rank], np.mean(rewards_task['SynthLoc']), np.mean(win_rate['SynthLoc']), metrics['total_loss'], batches))
            ...
            if (batches % config.eval_every == 0) and (local_rank == 0 or local_rank == 1):
                ...
                with open(f"log_results/BC_{config.run_name}_{medium_postfix}_noalign_{fliter_postfix}_seed_{config.seed}.csv", mode='w', newline='', encoding='utf-8') as file:
                    writer = csv.DictWriter(file, fieldnames=eval_metrics.keys())
                    writer.writeheader()  # 写入表头
                    writer.writerows(eval_metrics_list)  # 写入数据行
        ...
        # if (i % 10 == 0) and config.log_video and local_rank == 0:
        ...
        if (i % config.save_every == 0) and config.save_model and local_rank == 0:
            torch.save(agent.network.state_dict(), f"{config.model_saved_path}/Base-noalign-{config.run_name}-{medium_postfix}-{fliter_postfix}_seed{config.seed}_ep_{i}.pth")
            config_name = f"{config.model_saved_path}/Base-noalign-{config.run_name}-{medium_postfix}-{fliter_postfix}_config_seed{config.seed}.json"
